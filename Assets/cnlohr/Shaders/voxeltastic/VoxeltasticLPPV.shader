Shader "Unlit/VoxeltasticLPPV"
{
	...
	SubShader
	{
		Tags { "RenderType"="Transparent" "Queue"="Transparent+3" "LightMode"="ForwardBase" }
		...
		Pass
		{
			...
			#define unity_ProbeVolumeSH DUMP_TEXTURE
			...
			Texture2D< float4 > unity_ProbeVolumeSH;
			...
			void my_density_function( int3 pos, float distance, inout float4 accum )
			{
				...
				float4 col = 1.;
				col.rgb = SHEvalLinearL0L1_SampleProbeVolume(half4(float3(0,0,1), 1.0), (pos.xyz*float3(1./4.,.5,1./4.)-float3(16,2,16)));//ShadeSHPerPixel ( float3(0,0,1), 0., pos.xyz*int3(4,4,4));
			#else
				float4 col = .1;
			#endif

				//float4 col = unity_ProbeVolumeSH.Load( int4( pos.xyz*int3(4,4,4), 0.0 ) );
					//tex.Sample (sampler##samplertex,coord)
					//UNITY_SAMPLE_TEX3D_SAMPLER(unity_ProbeVolumeSH, unity_ProbeVolumeSH, pos.xyz/float3(32,4,32));
					//_Tex.Load( int4( pos.xyz, 0.0 ) ).a;
			...
			fixed4 frag (v2f i, uint is_front : SV_IsFrontFace ) : SV_Target
			{
				...
				int3 samplesize = int3( 32, 4, 32 )*4;
